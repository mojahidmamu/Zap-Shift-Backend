    ...
    try {
        ...
        app.post('/api/auth/register', async (req, res) => {
            try {
                ...
                const token = jwt.sign({ id: result.insertedId }, process.env.JWT_SECRET, { expiresIn: '30d' });
                res.status(201).json({
                token,
        ...
        app.put('/parcels/:id', async (req, res) => {
            try {
                ...
                const query = { _id: new ObjectId(id) };
                const result = await parcelCollection.updateOne(query, { $set: updatedData });
                res.json(result);
            } catch (error) {
                console.error(error);
                res.status(500).json({ error: error.message });
...
// ROOT ROUTE
app.get('/', (req, res) => {
    res.send('Zap Shift Backend and Server is  Running wit API and Database Connected');
});
